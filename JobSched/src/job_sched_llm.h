...
public:
	...
	void tokenize_prompt() {
		std::vector<llama_token> prompt_tokens(prompt.length() + 10);
		int n_prompt = llama_tokenize(vocab, prompt.c_str(), prompt.length(), prompt_tokens.data(), prompt_tokens.size(), false, false);
